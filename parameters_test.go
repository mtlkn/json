...
func TestParameters(t *testing.T) {
	...
	if pms[0].Value.String() != `["type","title"]` {
		fmt.Println(pms[0].Value.String())
		t.Error("failed to parse parameter value")
	}
	s2, v = setStringParameters(s, pms, nil)
	if s2 != `["type","title"]` || v.Type() != ArrayType || v.String() != `["type","title"]` {
		fmt.Println(v.String())
		t.Error("failed to set array parameters")
	}
	fmt.Printf("%s -> %s\n", s, s2)

	s2, v = setStringParameters("a", nil, nil)
	if s2 != "a" || v != nil {
		t.Fail()
		t.Error("failed to set unquoted parameter")
	}
}

func TestObjectParameters(t *testing.T) {
	s := `{ "name": "${name?YM}", "age": "${age:int}", "flag": "${flag:bool?true}", "salary": "${salary:float}", "url":"${domain?www.ap.org}/${action?story}/${id}", "a${p1?b}${p2?d}":123}`
	jo, err := ParseObjectWithParameters([]byte(s))
	if err != nil {
		t.Fail()
		t.Error(err.Error())
	}
	params, _ := ParseObject([]byte(`{"name":"SV", "age":27, "flag": true, "salary": 80000, "id":"xyz", "p2":"c"}`))
	...
	s = `{ "price": "$59.99"}`
	_, err = ParseObjectWithParameters([]byte(s))
	if err != nil {
		t.Fail()
		t.Error(err.Error())
	}
...
func TestObjectPrameters2(t *testing.T) {
	bs, err := os.ReadFile("testdata/obj-params.json")
	if err != nil {
		t.Fail()
		t.Error(err)
		return
	...
	jo, err := ParseObjectWithParameters(bs)
	if err != nil {
		t.Fail()
		t.Error(err)
		return
	...
	bs, err = os.ReadFile("testdata/params.json")
	if err != nil {
		t.Fail()
		t.Error(err)
		return
	...
	bs, err = os.ReadFile("testdata/res-params.json")
	if err != nil {
		t.Fail()
		t.Error(err)
		return
